...
memory_submit (gauge_t memory, virDomainPtr dom)
{
    ...
    char tsdb_tags[1024];
    ssnprintf(tsdb_tags,1024,"guest_name=%s",virDomainGetName(dom));
	meta_data_add_string (vl.meta,"tsdb_tags",tsdb_tags);
...
submit_derive2 (const char *type, derive_t v0, derive_t v1,
             virDomainPtr dom, const char *devname, enum tag_switch dev_type) 
{
    value_t values[2];
    value_list_t vl = VALUE_LIST_INIT;
	...
	switch (dev_type) {
		case DISK:
			ssnprintf(tsdb_tags,1024,"guest_name=%s disk_name=%s",virDomainGetName(dom),devname);
			break;
		case INTERFACE:
			ssnprintf(tsdb_tags,1024,"guest_name=%s if_name=%s",virDomainGetName(dom),devname);
			break;
	}
	meta_data_add_string (vl.meta,"tsdb_tags",tsdb_tags);
...
lv_read (void)
{
    ...
    for (i = 0; i < nr_block_devices; ++i) {
        ...
        if ((stats.rd_req != -1) && (stats.wr_req != -1))
            submit_derive2 ("disk_ops",
                    ...
                    block_devices[i].dom, block_devices[i].path, DISK);
	...
	if ((stats.rx_bytes != -1) && (stats.tx_bytes != -1))
	    submit_derive2 ("if_octets",
		    ...
		    interface_devices[i].dom, display_name, INTERFACE);

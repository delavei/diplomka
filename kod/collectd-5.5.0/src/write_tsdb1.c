...
static int wt_format_name(char *ret, int ret_len,
                          const value_list_t *vl,const char *ds_name )
{
	if (strcmp(vl->type,"disk_ops")==0 || 
		strcmp(vl->type,"disk_octets")==0 ||
		strcmp(vl->type,"if_octets")==0 ||
		strcmp(vl->type,"if_packets")==0 ||
		strcmp(vl->type,"if_errors")==0 ||
		strcmp(vl->type,"if_dropped")==0) {
		ssnprintf(ret, ret_len, "%s.%s.%s", vl->plugin, vl->type,
				ds_name);
	} else {
                          ...
                          const char *ds_name)
{
    ...
    if (ds_name != NULL) {
        if (vl->plugin_instance[0] == '\0') {
            if (vl->type_instance[0] == '\0') {
                ssnprintf(ret, ret_len, "%s%s.%s.%s   tato 1     ", prefix, vl->plugin,
                        vl->type, ds_name);
            } else {
                ssnprintf(ret, ret_len, "%s%s.%s.%s.%s   tato 2     ", prefix, vl->plugin,
                        vl->type, vl->type_instance, ds_name);
            }
        } else { // vl->plugin_instance != ""
            if (vl->type_instance[0] == '\0') {
                ssnprintf(ret, ret_len, "%s%s.%s.%s.%s   tato 3     ", prefix, vl->plugin,
                        vl->plugin_instance, vl->type, ds_name);
            } else {
                ssnprintf(ret, ret_len, "%s%s.%s.%s.%s.%s   tato 4     ", prefix,
        ...
        if (vl->plugin_instance[0] == '\0') {
            if (vl->type_instance[0] == '\0') {
                ssnprintf(ret, ret_len, "%s%s.%s   tato 5     ", prefix, vl->plugin,
                        vl->type);
            } else {
                ssnprintf(ret, ret_len, "%s%s.%s.%s   tato 6     ", prefix, vl->plugin,
                        vl->type_instance, vl->type);
            }
        } else { // vl->plugin_instance != "" 
            if (vl->type_instance[0] == '\0') {
                ssnprintf(ret, ret_len, "%s%s.%s.%s   tato 7     ", prefix, vl->plugin,
                        vl->plugin_instance, vl->type);
            } else {
                ssnprintf(ret, ret_len, "%s%s.%s.%s.%s   tato 8     ", prefix, vl->plugin,
                            ...
                            const char* host, meta_data_t *md)
{
	...
	fprintf (opentsdb_output,"%d %s",i,message);
	i++;
                             ...
                             struct wt_callback *cb)
{
    ...
    for (i = 0; i < ds->ds_num; i++)
    {
        ...
        if (status != 0)
        {
            ERROR("write_tsdb plugin: error with format_name1");
            return status;
        }
...
static int my_shutdown (void)
{
	...
	fprintf (opentsdb_output,"shutdown plugin opentsdb");
	fclose(opentsdb_output);
	return 0;
}

static int docasny_init()
{
    opentsdb_output = fopen("/home/juraj/opentsdb_output", "w" );
    fprintf (opentsdb_output,"%d \n",i);
	fflush(opentsdb_output);
    return 0;
}
...
void module_register(void)
{
    ...
    plugin_register_shutdown ("write_tsdb", my_shutdown);
    
    plugin_register_init("write_tsdb", docasny_init);
}
